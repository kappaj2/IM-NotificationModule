package za.co.ajk.notification.service.messaging.googlepubsub.impl;
...
import za.co.ajk.notification.config.MessageImplementationCondition;
import za.co.ajk.notification.config.PubSubMessagingProperties;
import za.co.ajk.notification.enums.PubSubMessageType;
import za.co.ajk.notification.service.messaging.IMMessageProcessor;
import za.co.ajk.notification.service.messaging.InterModulePubSubMessage;
import za.co.ajk.notification.service.messaging.googlepubsub.GoogleChannelManager;
...
public class GoogleChannelManagerImpl implements GoogleChannelManager {
    ...
    public PubSubInboundChannelAdapter messageChannelAdapterGenericSub(
        ...
        try {
            PubSubInboundChannelAdapter adapter =
                new PubSubInboundChannelAdapter(pubSubTemplate, "NotificationGenericSub");
            adapter
                .setOutputChannel(
    ...
    public PubSubInboundChannelAdapter messageChannelAdapterBillingSub(
        ...
        try {
            PubSubInboundChannelAdapter adapter =
                new PubSubInboundChannelAdapter(pubSubTemplate, "NotificationTopicSub");
            adapter
                .setOutputChannel(
                    inputChannel); // looks like the channel to ack on (thus the input channel - confusing!)
            adapter.setAckMode(AckMode.MANUAL);
